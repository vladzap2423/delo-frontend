          ...
          >
            ...
            {users.map((u, idx) => (
              <Rnd
                ...
                minHeight={70}
                bounds="parent"
                dragGrid={[10, 10]}   // привязка к сетке (горизонталь и вертикаль по 10px)
                resizeGrid={[10, 10]} // ресайз тоже с шагом
                style={{
                ...
                onResizeStop={(e, dir, ref, delta, pos) => {
                  ...
                  slotsRef.current = slotsRef.current.map((s) =>
                    s.userId === u.id
                      ? { ...s, x: pos.x, y: pos.y, width: newWidth, height: newHeight }
                      : s
                  );
                }}
              >
                <SignatureBlock fio={u.fio} preview />
              </Rnd>
            ))}
