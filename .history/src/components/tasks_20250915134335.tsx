...
export default function TaskPage() {
  ...
  const renderTaskCard = (task: Task) => (
    ...
    >
      {/* Иконка */}
      <img src="/icons/icon_documents.png" alt="doc" className="w-8 h-8 mt-1" />
          ...
          {task.signs.map((s) => (
            <span
              ...
              className={`px-2 py-1 rounded text-sm text-white ${
                s.isSigned ? "bg-green-600" : "bg-red-600"
              }`}
            >
              {s.user.fio}
            </span>
        ...
        <div className="space-y-3">
          {inProgress.map((task) => (
            ...
            >
              {/* Иконка */}
              <img
                src="/icon_documents.png"
                alt="doc"
                className="w-32 h-32"
              />
              {/* Контент */}
              <div className="flex flex-col flex-1 text-left">
                <span className="font-bold text-lg text-center">{task.title}</span>
